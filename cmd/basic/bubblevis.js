function createVis() {
    ...
    function overallVis(incomingData) {
        ...
        var maxS = d3.max(incomingData, d => Number(d[scatterS]))
        var minS = d3.min(incomingData, d => Number(d[scatterS]))  
        ...
        if (scatterC) {
  	    ...
  	    cScale = d3.scaleQuantize().domain([minC, maxC]).range(colorbrewer.Set2[6]);
        }
